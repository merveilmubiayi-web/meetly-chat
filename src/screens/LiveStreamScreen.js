...
export default function LiveStreamScreen({ navigation }) {
  ...
  return <View style={[styles.container, themeStyles.screen]}><Text style={[styles.title, themeStyles.text]}>Démarrer un live</Text><Text style={[styles.text, themeStyles.secondaryText]}>Le flux vidéo est transporté par LiveKit.</Text><TouchableOpacity style={styles.button} onPress={start} disabled={starting}><Text style={styles.buttonText}>{starting ? 'Connexion...' : 'Démarrer le live'}</Text></TouchableOpacity></View>;
}
