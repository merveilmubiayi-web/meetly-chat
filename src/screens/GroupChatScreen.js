...
export default function GroupChatScreen({ route }) {
  ...
  return <View style={[styles.container, themeStyles.screen]}><FlatList data={messages} keyExtractor={(item) => item.id} renderItem={({ item }) => <View style={[styles.message, themeStyles.card]}><Text style={styles.sender}>{item.sender_id}</Text><Text style={[styles.body, themeStyles.text]}>{item.body}</Text></View>} /><View style={styles.inputRow}><TextInput style={[styles.input, themeStyles.input]} value={body} onChangeText={setBody} placeholder="Message" placeholderTextColor={themeStyles.theme.textSecondary} /><TouchableOpacity style={styles.button} onPress={send}><Text style={styles.buttonText}>Envoyer</Text></TouchableOpacity></View></View>;
}
