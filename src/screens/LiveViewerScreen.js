...
export default function LiveViewerScreen({ route, navigation }) {
  ...
  return <View style={[styles.container, themeStyles.screen]}><Text style={[styles.title, themeStyles.text]}>Live</Text><Text style={[styles.text, themeStyles.secondaryText]}>Les spectateurs rejoignent la salle LiveKit.</Text><TouchableOpacity style={styles.button} onPress={join} disabled={joining}><Text style={styles.buttonText}>{joining ? 'Connexion...' : 'Rejoindre le live'}</Text></TouchableOpacity></View>;
}
