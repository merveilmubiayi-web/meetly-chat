...
export default function CreateGroupScreen({ navigation }) {
  ...
  return <View style={[styles.container, themeStyles.screen]}><Text style={[styles.title, themeStyles.text]}>Créer un groupe</Text><TextInput style={[styles.input, themeStyles.input]} value={title} onChangeText={setTitle} placeholder="Nom du groupe" placeholderTextColor={themeStyles.theme.textSecondary} /><FlatList data={users} keyExtractor={(item) => item.id} renderItem={({ item }) => <TouchableOpacity style={[styles.row, { borderBottomColor: themeStyles.theme.border }]} onPress={() => toggle(item.id)}><Text style={[styles.name, themeStyles.text]}>{item.name || item.username || item.id}</Text><Text style={styles.check}>{selected.includes(item.id) ? '✓' : ''}</Text></TouchableOpacity>} /><TouchableOpacity style={styles.button} onPress={create} disabled={saving}><Text style={styles.buttonText}>{saving ? 'Création...' : 'Créer le groupe'}</Text></TouchableOpacity></View>;
}
